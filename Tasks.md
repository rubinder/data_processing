...
- [x] `impression_quality_checks` works in `SPARK_MODE=aws`: `quality_checks.AwsQualitySource` reads the DynamoDB status table and the S3 manifests, `LocalQualitySource` the local layout; the rules are storage-agnostic (`freshness_from_records`, `volume_from_manifests`); 18 tests incl. botocore-stubbed AWS reads
- [x] Athena lineage emitted from the pipeline: `VerifyInAthena` state invokes `lambda/athena_lineage.py`, which runs the partition count through the workgroup and posts OpenLineage START/COMPLETE (or FAIL) with SQL, bytes-scanned and row-count facets and a parent facet naming the Step Function run; verified live 2026-09-08 (68,367 rows, 0 bytes scanned)
- [x] `hello_world.run()` returns the collected rows and the test asserts on them (the Table API prints from the JVM, outside capsys)
- [x] Glue crawler targets are now `raw/impressions/` and `processed/`; catalog holds `impressions` and `processed` partitioned by page_type/date/hour; Step Function recrawls after the ETL; Athena query on `processed` returns 68,367 rows (2026-09-07)
- [x] Lineage on Glue/EMR verified live 2026-09-08 through the stack's optional capture endpoint (`LineageSinkEnabled`, API Gateway + Lambda) and replayed into local Marquez (`lineage_deployment/replay_cloudwatch.py`): Glue emits processed/quarantine writes with S3 inputs/outputs; EMR emits through the shipped jar (`EmrOpenLineageJar=shipped` default, Maven now opt-in)
- [x] EMR split into its own stack (`cloudformation/emr.yaml`, `scripts/emr.sh up|status|down`); the core stack `data-processing-pipeline` stays up at ~$0 idle, the EMR stack is created per job and deleted after (verified create + delete 2026-09-07). Remove everything with `scripts/emr.sh down` then `aws cloudformation delete-stack --stack-name data-processing-pipeline` (empty the landing/processed buckets first)

## Pinecone (added 2026-09-08)
- [x] `pinecone_deployment/`: corpus generator, embedders (hash / sentence-transformers / Pinecone Inference), store (deterministic ids, batched retrying upserts, namespace vs filter isolation, model tag + `__meta__` record), hashed BM25 + hybrid weighting, rerankers, evaluation harness, migration flow, FastAPI, docker-compose with Pinecone Local, deploy.sh, 20 tests
- [x] Local benchmark (2,000 conversations, 20 tenants): namespace isolation 2x faster than filter at equal results; batch size sweet spot 100-200; lexical rerank does not help; Pinecone Local does not score sparse vectors (hybrid ranking is cloud-only); freshness and offboarding measured
...
set of approved query templates, and an MCP server that can run only those.
    ...
    - [ ] Connection pool for the MCP server and a concurrent-load measurement
