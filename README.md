...
| [`spark_applications/`](spark_applications/) | PySpark jobs: API ingestion, partitioned writes, aggregation, skew handling via salting, plus a `DECISIONS.md` on performance and idempotency |
| [`flink_applications/`](flink_applications/) | PyFlink jobs, including a Debezium CDC consumer with exactly-once checkpointing and event-time watermarks |
| [`airflow_deployment/`](airflow_deployment/) | Airflow 2 via Docker: backfillable hourly DAG, dynamic task mapping, retries and SLAs |
| [`aws_deployment/`](aws_deployment/) | CloudFormation: EMR, S3 events, Lambda, Step Functions, Glue, Athena, DynamoDB |
...
| [`debezium_deployment/`](debezium_deployment/) | Postgres → Debezium → Kafka CDC stack |
| [`dbt_deployment/`](dbt_deployment/) | dbt + PostgreSQL: staging, intermediate, and four analytical models with tests |
